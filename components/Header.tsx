            ...
            <div className="space-y-1 pb-6 pt-2">
              {navigationItems.map((item) => (
                <Link
                  key={item.key}
                  href={item.href}
                  className="block py-3 text-base font-medium text-foreground/80 transition-colors duration-200 hover:text-foreground"
              ...
              <Link
                href="/contact"
                className="mt-4 block w-full rounded-none bg-foreground px-6 py-3 text-center text-base font-medium text-white"
                onClick={() => setMobileMenuOpen(false)}
              >
                {tNav("programare")}
              </Link>
