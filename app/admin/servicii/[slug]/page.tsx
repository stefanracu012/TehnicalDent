...
export default function EditServicePage({
  ...
  const fetchService = async () => {
    try {
      ...
      const cats = [
        ...new Set(
          services.map((s: { category: string }) => s.category).filter(Boolean),
        ),
      ] as string[];
        ...
        for (const loc of Object.keys(loadedTranslations)) {
          if (typeof loadedTranslations[loc].process === "string") {
            loadedTranslations[loc].processSteps = (
              ...
              .filter((s: string) => s.trim().length > 0)
              .map((s: string) => s.trim().replace(/\.$/, ""));
            delete loadedTranslations[loc].process;
          }
        }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={formData.slug}
                  className="w-full border border-border px-4 py-3 bg-muted focus:outline-none"
                  readOnly
                />
              </div>
            </div>
